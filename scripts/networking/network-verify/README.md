...
2. **Platform Detection**: Automatically detects the cloud provider (AWS or GCP) from cluster infrastructure.  Note that a flag
must be passed for HCP or Zero-Egress platform types
3. **Region Detection**: Automatically detects the region for AWS-based clusters (classic, HCP, or HCP zero egress)
4. **Pod-based Verification**: Runs network verification using Kubernetes Jobs in the `openshift-network-diagnostics` namespace
5. **Service Log Skip**: Uses the `--skip-service-log` flag to prevent automatic service log generation
6. **Cleanup**: Automatically cleans up temporary files after completion
