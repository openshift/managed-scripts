...
collect_manifests() {
  ...
  hc_namespace=$(oc get namespace -l api.openshift.com/id="${CLUSTER_ID}" -o json | jq -r '.items[0].metadata.labels["kubernetes.io/metadata.name"]')
  ...
  # local acm_hypershift_image="registry.redhat.io/rhacm2/acm-must-gather-rhel9:v2.11"
  local acm_hypershift_image="quay.io/rokejungrh/must-gather:v2.13.0-33-linux"
